...
for (let index = 0; index < input.length; index+=3) {
    const first = input[index].split('');
    const second = input[index+1].split('');
    const third = input[index+2].split('');
    
    const firstUnique = Object.keys(first.reduce((acc, curr) => {
        acc[curr] = true;
        return acc;
    }, {}));
    const secondUnique = Object.keys(second.reduce((acc, curr) => {
        acc[curr] = true;
        return acc;
    }, {}));
    const thirdUnique = Object.keys(third.reduce((acc, curr) => {
        acc[curr] = true;
        return acc;
    }, {}));

    const common = firstUnique.filter(f => secondUnique.find(s => s === f)).filter(f => thirdUnique.find(s => s === f));
    
    const priority = common[0] === common[0].toLowerCase() ? common[0].charCodeAt(0) - 96 : common[0].charCodeAt(0) - 38;
    result += priority;
}
